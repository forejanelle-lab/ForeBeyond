-- Background check is consent-only (auto-verified).
-- Fix profile verification_status when only non-ID docs were rejected.

CREATE OR REPLACE FUNCTION public.auto_verify_background_check_consent()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $$
BEGIN
  IF NEW.document_type = 'background_check' THEN
    NEW.status := 'verified';
    NEW.reviewed_at := COALESCE(NEW.reviewed_at, NOW());
    NEW.notes := COALESCE(NULLIF(TRIM(NEW.notes), ''), 'Background check consent recorded.');
  END IF;
  RETURN NEW;
END;
$$;
...
CREATE TRIGGER auto_verify_background_check_consent
  BEFORE INSERT OR UPDATE ON verification_documents
  FOR EACH ROW
  WHEN (NEW.document_type = 'background_check')
  EXECUTE FUNCTION auto_verify_background_check_consent();

UPDATE verification_documents
SET
  status = 'verified',
  reviewed_at = COALESCE(reviewed_at, NOW()),
  notes = COALESCE(NULLIF(TRIM(notes), ''), 'Background check consent recorded.'),
  updated_at = NOW()
WHERE document_type = 'background_check'
  AND status IN ('pending', 'in_review', 'unverified');
...
BEGIN
  ...
  ELSIF EXISTS (
    ...
    WHERE user_id = p_user_id
      ...
      AND document_type <> 'background_check'
  ) THEN
    UPDATE profiles
...
-- Profiles should not stay "rejected" when government ID is verified.
UPDATE profiles p
SET verification_status = 'verified', updated_at = NOW()
WHERE verification_status = 'rejected'
  AND EXISTS (
    ...
    WHERE vd.user_id = p.id
      AND vd.document_type = 'government_id'
      AND vd.status = 'verified'
  );

-- Award background_checked badge in trust score calculation.
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM verification_documents
    WHERE user_id = p_user_id AND document_type = 'background_check' AND status = 'verified'
  ) THEN
    INSERT INTO trust_badges (user_id, badge_type) VALUES (p_user_id, 'background_checked')
    ON CONFLICT (user_id, badge_type) DO NOTHING;
  END IF;
  IF score >= 70 THEN
...
SELECT calculate_trust_score(id)
FROM profiles
WHERE id IN (
  SELECT DISTINCT user_id FROM verification_documents WHERE document_type = 'background_check'
);
